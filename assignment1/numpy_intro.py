...
#        set of "true" feature weights. This is the feature weights our linear regression
...
# HINT: Note that matrices in numpy can also be represented with numpy arrays.
...
# by the magnitude, not the direction of error (i.e. the score function treats an error of +1 and -1 the same).
# 2) Larger errors are penalized exponentially more than smaller errors (e.g. an error of 4 contributes
# 16 to the score function, while an error of 2 only contributes 4).
#
# Score functions are not only used to evaluate the model, but also to build the model. Different score functions build
# different models. Luckily for us, linear regression for the least squares regression score function
...
#       use a loop, which would take around 5 lines of code.
